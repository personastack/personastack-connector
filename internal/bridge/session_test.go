...
func TestConnectFrameSignsAPIVerifiableMessage(t *testing.T) {
	...
	if frame.Connect.ProtocolVersion != externalagentprotocol.ProtocolVersionV4 {
		t.Fatalf("protocol version: got=%s", frame.Connect.ProtocolVersion)
	}
	if len(frame.Connect.SupportedProtocolVersions) != 1 || frame.Connect.SupportedProtocolVersions[0] != externalagentprotocol.ProtocolVersionV4 {
