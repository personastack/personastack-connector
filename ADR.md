...
- 2026-08-01: PersonaStack API owns external-persona account/profile selection. The Connector reports safe opaque inventory candidates and resolves an API-provided target only for its current session. It must not persist an account, profile, home path, or OpenClaw agent id. A root system Connector may launch the selected runtime under that account. A user-scope Connector may launch only its effective account.
- 2026-08-02: A paired Connector session is an always-on bridge, not a runtime health transaction. V4 keeps the websocket alive while a session-owned reconciler retries target resolution, runtime startup, MCP reconciliation, and live verification. Generation, selection revision, and target epoch fence every publish, wake probe, and run admission. Target clear is an explicit capability-marked protocol operation. Runtime faults remain typed and redacted until recovery is observed automatically.
